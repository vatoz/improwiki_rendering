...
\needspace{5cm} \section{Barman song} \label{barman song} \katabox{Problém pro každého hráče 
}{neomezeně 
}{neomezeně} 
...
\needspace{5cm} \section{Opilecká píseň} \label{opilecká píseň} \katabox{Téma 
}{4 
}{volný} 
 
V kategorii \textbf{Opilecká píseň}{} se hráči vcítí do opilosti a zazpívají vám na vaše téma píseň jako by byli v hospodě nebo při nočním návratu z hospody domů. 
 
 
\subsection{ Průběh } 4 \odkaz{hráči}{hráč} se postaví vedle sebe. \odkaz{Hudebník}{hudebník} začne hrát jednoduchou melodii a začíná se generickým refrénem na čtyři doby, ve kterém se nezpívá nic určitého, jen "hajdy dajdy". Refrén se vždy opakuje dvakrát. Po skončení refrénu se veršovaně (\odkaz{ABAB}{abab}) zpívá sloka, každý \odkaz{hráč}{hráč} zazpívá jeden verš. Sloku začíná hráč stojící vlevo a pokračuje se po řadě. Po skončení sloky se opakuje refrén. Po skončení refrénu začíná hráč sloku o jedna vpravo a vše se opakuje, dokud se se začínáním nevystřídají všichni. Píseň končí opět refrénem. 
 
\subsection{ Varianty } \begin{itemize}
\item Refrén má slova
\item Sloky se jednou opakují
\item Sloky jsou delší o 4 doby
\end{itemize}
 
\subsection{ Běžné chyby } \begin{itemize}
\item Nikdo neví, kdo má začínat sloku
\item Hudebník hraje příliš rychlou melodii a hráči nestíhají
\item Hráči stejně nestíhají
\end{itemize}
...
V kategorii \textbf{Počet slov}{} známé též jako \textbf{7 slov}{} či '''1, 3, 5, 7' hráči  budou přesně dodržovat počet slov ve větě. 
...
\needspace{5cm} \section{Prskavka ala šanson} \label{prskavka ala šanson} \katabox{Reálné předměty z publika 
}{libovolný, každý jednotlivě 
}{cca 40 - 90 vteřin na každého hráče 
} 
V kategorii \textbf{Prskavka ala šanson}{} Vám hráči zazpívají o svém silném vztahu k předmětům, které nám teď zapůjčíte.  
 
\subsection{Průběh} Varianta \odkaz{prskavky}{prskavka}. Hráči stojí zády k publiku, \odkaz{MC}{mc} si vyžádá od publika zapůjčení zajímavých předmětů, připraví mikrofon. 
Vyvolá vždy jednoho hráče, ten bere mikrofon a  zpívá "monolog"{} o svém silném vztahu k předmětu.  
 
\subsection{Varianty} \begin{itemize}
\item  Píseň je možno pojmout šansonově, tedy uvést píseň melodramatickým hlasem, přejít do šansonu o tom, jak mi tento předmět změnil život.
\item  Ostatní hráči vytvářejí zpěvákovi chór, podobně jako je tomu v kategorii \odkaz{zpívaná}{zpívaná}.
\end{itemize}
 
\subsection{Běžné chyby} \begin{itemize}
\item Zdlouhavé vybírání předmětů.
\item Hráč nemá k předmětu vztah.
\end{itemize}
...
\needspace{5cm} \section{Ruce} \label{ruce} \katabox{Bláznivý vynález 
}{4+ (nebo 4 a MC) 
}{cca 8 minut} 
 
 
V kategorii \textbf{Ruce}{}  se experti budou snažit přijít na to, co vlastně vynalezli. 
 
\subsection{Průběh} Dva hráči, experti, se vzdálí a \odkaz{MC}{mc} spolu s diváky určí bláznivý vynález, který experti vynalezli. 
Po návratu expertů každému expertovi provlíkne ruce pod pažemi, expert dá své ruce dozadu a jeho ruce hraje druhý hráč. 
 
\odkaz{MC}{mc} klade expertům otázky se zvyšující se návodností, cílem je, aby experti popsali svůj vynález. 
 
\subsection{ Běžné chyby } \begin{itemize}
\item Kategorie trvá příliš dlouho (není nutno dohrávat)
\end{itemize}
...
\subsection{ Běžné chyby } \begin{itemize}
\item Hráči nesledují čas. (Pomocňák ukazuje čas pro každý minutový úsek i pro závěrečnou půlminutu samostatně.)
\end{itemize}
